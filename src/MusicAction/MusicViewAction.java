...
public class MusicViewAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int idx = Integer.parseInt( request.getParameter("m_no") );
		...
		int totalSize = CommentsDAO.getInstance().CommentsCount(Integer.parseInt(request.getParameter("m_no")) );
		Paging paging = new Paging( pageSize, totalSize, currentPage );
		
		HashMap<String, Integer> map = new HashMap<>();
		map.put("startNo", paging.getStartNo());
		map.put("endNo", paging.getEndNo());
		map.put("memberidx",   Integer.parseInt(request.getParameter("m_no")) );
		List<CommentsVO> list = CommentsDAO.getInstance().select(map);
		request.setAttribute("paging", paging);
		
		request.setAttribute("name",name);
		request.setAttribute("idx", Integer.parseInt(request.getParameter("m_no")) );
